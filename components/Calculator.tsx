...
export const Calculator: React.FC<CalculatorProps> = ({ 
  ...
  const handleMemory = (op: 'M+' | 'M-' | 'MR' | 'MC') => {
    const currentVal = parseFloat(eval(value) || '0'); // Simple eval for memory, careful with complex expr
  ...
  const SpaceIcon = () => (
    <div className="flex items-center justify-center pt-2">
      <div className="w-5 h-1.5 border-b-2 border-x-2 border-current opacity-70 rounded-bl-[2px] rounded-br-[2px]"></div>
    </div>
  );
  ...
  const BASIC_KEYS: CalculatorButton[] = [
    ...
    { label: <SpaceIcon />, value: ' ', type: 'number', shiftLabel: '×10ˣ', shiftValue: '*10^' },
    { label: 'Ans', value: 'Ans', type: 'number', shiftLabel: 'Pre', shiftValue: 'preAns' },
    { label: '=', value: '=', type: 'action', shiftLabel: '≈', shiftValue: 'approx', alphaLabel: '%', alphaValue: '%' },
  ];
  ...
  const getBtnClasses = (btn: CalculatorButton) => {
    ...
    // Specific Overrides
    if (btn.value === '=') colors = `${theme.btnEqual} text-xl font-bold`; // Make equal sign bigger/bold
